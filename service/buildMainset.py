#!/usr/bin/python -tt
import sys, pymongo, os, glob, re, bson.json_util
...
def run(collection=None):
	...
	try:
		if collection: 
			...
			fullcoll = fulldb[collection]			
		else:
			fulldb = client["xmlVis"]
			fullcoll = fulldb["listfiles"]	
		
		listOfRoot = fullcoll.find({"type":"tcga"}).distinct("root");
		listOfRoot.sort()
		rootarr = []
		...
		for listRoot in listOfRoot:
			...
			listLocation = fullcoll.find({"type":"tcga", "root":subRoot}).distinct("location")
			locationarr = []
			listLocation.sort()
			for location in listLocation:
				...
				listType = fullcoll.find({"type":"tcga", "root":subRoot, "location":location}).distinct("roottype")
				listType.sort()
				typearr = []
				for typeVal in listType:
					...
					listSub = fullcoll.find({"type":"tcga", "root":subRoot, "location":location, "roottype":typeVal}).distinct("subtype")
					listSub.sort()
					subArr = []
					for subType in listSub:
						...
						listForward = fullcoll.find({"type":"tcga", "root":subRoot, "location":location, "roottype":typeVal, "subtype":subType})
						forwardArr = []
